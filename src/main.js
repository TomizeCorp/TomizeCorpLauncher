...
async function installAndLaunch(win, profile) {
  ...
  const skinPath=account?.skinPath&&fsSync.existsSync(account.skinPath)?account.skinPath:'';
  const child = await launch({ gamePath: settings.instancePath, javaPath, version: fabricVersion, versionName: 'EpsilonLauncher', versionType: 'EpsilonLauncher', gameName: 'EpsilonLauncher', gameProfile: { name: username, id: microsoft ? activeSession.id : offlineUuid(username) }, accessToken: microsoft ? activeSession.accessToken : '0', userType: microsoft ? 'mojang' : 'legacy', launcherName: 'EpsilonLauncher', launcherBrand: 'TomizeCorp', minMemory: 1024, maxMemory: 4096, extraJVMArgs:skinPath?[`-Depsilon.skin=${skinPath}`,`-Depsilon.username=${username}`]:[], quickPlayMultiplayer: `${settings.serverAddress}:${settings.serverPort}`, server: { ip: settings.serverAddress, port: settings.serverPort }, extraExecOption: { detached: true } });
