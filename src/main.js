...
app.whenReady().then(() => {
  ...
  ipcMain.handle('server:status', () => accountApi('/v1/server-status'));
  ...
  ipcMain.handle('auth:register', async (_, value) => {const credentials={...validateCredentials(value),email:String(value?.email||'').trim()};return setRemoteSession(await accountApi('/v1/register',{method:'POST',body:credentials}),value?.rememberSession!==false);});
  ipcMain.handle('auth:forgot-password', async (_, email) => accountApi('/v1/password/forgot',{method:'POST',body:{email}}));
  ipcMain.handle('auth:reset-password', async (_, value) => accountApi('/v1/password/reset',{method:'POST',body:value}));
  ...
  ipcMain.handle('account:get', async () => {const s=await loadSettings();if(!s.authMode)return null;if(s.authMode==='microsoft'){if(!s.displayName)return null;return{mode:'microsoft',username:s.displayName,officialSkin:true,preview:await officialSkinPreview(s.profileId,s.displayName)}}const session=await remoteSession(),account=session.account;let preview='';if(account.skin){const data=Buffer.from(account.skin,'base64'),skinsDir=path.join(app.getPath('userData'),'skins');await fs.mkdir(skinsDir,{recursive:true});const target=path.join(skinsDir,`${account.id}.png`);await fs.writeFile(target,data);activeSession.skinPath=target;preview=`data:image/png;base64,${account.skin}`;}return{mode:'epsilon',username:account.username,email:account.email||'',hasSkin:account.hasSkin,preview};});
