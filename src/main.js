...
app.whenReady().then(() => {
  ...
  ipcMain.handle('auth:login', async (_, value) => { const {username}=validateCredentials(value);activeSession={type:'offline',name:username};const s=await loadSettings();await saveSettings({...s,username,displayName:username,authMode:'epsilon'});return{name:username,mode:'epsilon'}; });
